...
class RenderUtilTest(absltest.TestCase):
  ...
  @absltest.skipUnless(test_data.supports_cubql(), "Skipping test that requires cuBQL BVH.")
  @absltest.skipUnless(
    test_data.supports_texture(), "Skipping test that requires CUDA textures (unsupported on HIP/ROCm)."
  )
